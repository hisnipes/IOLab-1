...
def submitSurvey():
	...
	if 'username' in session:
		...
		model.persist_in_mongo(username, email, surveyResponse)
...
def aggregateData():
	feBefore = model.aggregate_feBefore()
	feAfter = model.aggregate_feAfter()
	userGroup = model.aggregate_userGroup()
	return render_template('aggregate.html', feBefore=feBefore, feAfter=feAfter, userGroup=userGroup)
